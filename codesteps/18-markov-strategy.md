...
- **Training corpus is stored as a lexicon-like JSON file** (a list of training words), not raw text — so it goes through the same lazy-load path as everything else. Concretely, the example pack's corpus is the *existing* `elven-given` lexicon (step 01's fixture data), reused as-is via `lexiconRefs: { corpus: "elven-given" }` rather than authoring new fictional content — no new lexicon file needed.
